...
from atom.config.errors import ConfigIssue, ConfigLoadError, validation_issues
...
def _migrate_legacy_home_dir(home_dir: Path) -> None:
    """Adopt a pre-rename ``~/.nanobot`` directory as ``~/.atom``, once.

    Atom was forked from nanobot, which stored config, sessions, memory and
    channel auth under ``~/.nanobot``. Renaming the project would otherwise
    orphan that state. Only migrate when the new directory does not yet exist,
    so an existing Atom install is never touched.
    """
    if home_dir.exists():
        return
    legacy = home_dir.parent / ".nanobot"
    if not legacy.is_dir() or legacy.is_symlink():
        return
    with suppress(OSError):
        legacy.rename(home_dir)
...
def get_config_path() -> Path:
    ...
    home_dir = Path.home() / ".atom"
    _migrate_legacy_home_dir(home_dir)
        ...
        try:
            ...
            raise ConfigLoadError(
                ...
                summary=(
                    ...
                    "Check that complex ATOM_* values use valid JSON."
                ),
            ) from exc
...
def _apply_ssrf_whitelist(config: Config) -> None:
    ...
    from atom.security.network import configure_ssrf_whitelist
