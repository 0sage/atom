...
from atom.config.timezone import detect_system_timezone
from atom.config_base import Base
from atom.cron.types import CronSchedule

if TYPE_CHECKING:
    from atom.agent.tools.cli_apps import CliAppsToolConfig
    from atom.agent.tools.filesystem import FileToolsConfig
    from atom.agent.tools.image_generation import ImageGenerationToolConfig
    from atom.agent.tools.self import MyToolConfig
    from atom.agent.tools.shell import ExecToolConfig
    from atom.agent.tools.web import WebToolsConfig
...
class TranscriptionConfig(Base):
    ...
    enabled: bool = True
    provider: str | None = None  # Validated by atom.audio.transcription_registry.
    model: str | None = None
...
class ModelPresetConfig(Base):
    ...
    def to_generation_settings(self) -> Any:
        from atom.providers.base import GenerationSettings
...
class AgentDefaults(Base):
    ...
    workspace: str = "~/.atom/workspace"
    model_preset: str | None = None  # Active preset name — takes precedence over fields below
    model: str = "anthropic/claude-opus-4-5"
    provider: str = (
        "auto"  # Provider name (e.g. "anthropic", "openai") or "auto" for auto-detection
    )
    max_tokens: int = 8192
    context_window_tokens: int = 200_000
    ...
    bot_name: str = "atom"  # Display name shown in CLI prompts (e.g. "{name} is thinking...")
...
class ProviderConfig(Base):
    ...
    # atom/providers/openai_compat_provider.py. Kept duplicated here to
...
class ToolsConfig(Base):
    ...
    web: WebToolsConfig = Field(default_factory=lambda: _lazy_default("atom.agent.tools.web", "WebToolsConfig"))
    exec: ExecToolConfig = Field(default_factory=lambda: _lazy_default("atom.agent.tools.shell", "ExecToolConfig"))
    file: FileToolsConfig = Field(default_factory=lambda: _lazy_default("atom.agent.tools.filesystem", "FileToolsConfig"))
    cli_apps: CliAppsToolConfig = Field(default_factory=lambda: _lazy_default("atom.agent.tools.cli_apps", "CliAppsToolConfig"))
    my: MyToolConfig = Field(default_factory=lambda: _lazy_default("atom.agent.tools.self", "MyToolConfig"))
    image_generation: ImageGenerationToolConfig = Field(
        default_factory=lambda: _lazy_default("atom.agent.tools.image_generation", "ImageGenerationToolConfig"),
    )
    restrict_to_workspace: bool = False  # policy intent: keep tool access inside workspace when possible
    # Gates shell-tool access to localhost services (see atom/agent/tools/shell.py
    # and atom/channels/validation.py). The "webui" prefix and the legacy
    # allowLocalPreviewAccess aliases are kept so existing config files keep loading.
...
class Config(BaseSettings):
    ...
    def _match_provider(
        ...
        *,
        ...
        from atom.providers.registry import (
            PROVIDERS,
            find_by_name,
        )
        ...
        # Explicit provider prefix wins over a keyword match elsewhere in the model name.
...
def _resolve_tool_config_refs() -> None:
    ...
    like ``from atom.config.schema import ExecToolConfig`` continue to work.
    """
    import sys
    ...
    from atom.agent.tools.self import MyToolConfig
    from atom.agent.tools.shell import ExecToolConfig
    from atom.agent.tools.web import WebFetchConfig, WebSearchConfig, WebToolsConfig
