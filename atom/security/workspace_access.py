...
class WorkspaceScopeResolver:
    ...
    # Channel allowed to request a per-turn workspace scope (project path +
    # access mode) through message metadata. No shipped channel is trusted for
    # that today, so this is opt-in only: with None, every turn resolves to the
    # configured default scope and client-supplied metadata is ignored.
    ...
    def _is_scoped_channel(self, channel: str | None) -> bool:
        return self.scoped_channel is not None and channel == self.scoped_channel
    ...
    def for_turn(
        ...
        *,
        ...
        if not self._is_scoped_channel(channel):
            return self.default()
    ...
    def persist_message_scope(self, session: Any, msg: Any) -> None:
        if not self._is_scoped_channel(getattr(msg, "channel", None)):
            return
...
def workspace_sandbox_status(
    *,
    ...
    return WorkspaceSandboxStatus(
        ...
        provider="none",
        provider_label=_provider_label("none"),
        summary="Workspace restriction uses atom application-level guards.",
    )
...
def current_scope_allows_loopback(*, enabled: bool) -> bool:
    ...
    Requires a scope that a trusted channel explicitly requested for this turn
    (``source_channel`` set by ``WorkspaceScopeResolver.scoped_channel``), not
    merely an unrestricted default workspace. Default turns carry no
    ``source_channel``, so loopback stays blocked for them.
    """
    ...
    return bool(
        ...
        and scope.source_channel is not None
...
def _env_system_provider(environ: dict[str, str] | None = None) -> str | None:
    ...
    explicit_provider = env.get("ATOM_WORKSPACE_SANDBOX_PROVIDER")
    enforced = env.get("ATOM_WORKSPACE_SANDBOX_ENFORCED")
    compatibility = env.get("ATOM_SANDBOX_ENFORCED")
