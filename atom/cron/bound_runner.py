...
from atom.cron.session_delivery import origin_delivery_context
...
def _bound_session_delivery_context(job: CronJob) -> tuple[str, str, dict[str, Any]]:
    channel, chat_id, metadata = origin_delivery_context(job)
    # Origin metadata is persisted per job and may predate the WebUI removal.
    # Drop the stale turn identity so a proactive delivery never reuses the
    # turn id of the conversation that created the job.
