...
if TYPE_CHECKING:
    ...
    from atom.providers.registry import ProviderSpec
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/0sage/atom",
    "X-OpenRouter-Title": "atom",
...
# because it does not support thinking. Reachable only when a custom provider
# key is named so that its dynamic spec matches one of these model names.
...
def _openai_compat_timeout_s() -> float:
    ...
    return _float_env("ATOM_OPENAI_COMPAT_TIMEOUT_S", _OPENAI_COMPAT_REQUEST_TIMEOUT_S)
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply Atom attribution headers to OpenRouter requests by default."""
...
class OpenAICompatProvider(LLMProvider):
    ...
    def __init__(
        ...
        # to create. Defer until first use.
                ...
                if os.environ.get("LANGFUSE_SECRET_KEY") and importlib.util.find_spec("langfuse"):
                    ...
                    if os.environ.get("LANGFUSE_SECRET_KEY"):
                        logger.warning(
                            ...
                            "run `atom plugins enable langfuse` to enable tracing"
                        )
    ...
    def _build_kwargs(
        ...
        # otherwise lets extra_body.tools replace atom's generated functions.
        if self._extra_body:
    ...
    def _handle_error(
        ...
        *,
        ...
        # Gate on the endpoint URL rather than a spec flag: a local deployment now
        # reaches us as a `custom` provider pointed at localhost, not a dedicated spec.
        if _is_local_endpoint(spec, api_base) and (
            ...
            msg += (
                ...
                f"{api_base or (spec.default_api_base if spec else '')}, and if you are using a "
                "proxy/tunnel, make sure it can reach your local service instead of routing "
                "localhost through the remote host."
            )
