...
def sanitize_surrogates(text: Any) -> Any:
    ...
    UTF-16 to reconstruct genuine surrogate pairs (produced e.g. by console
    input for emoji) and substitutes lone surrogates with
    ``U+FFFD``.
...
_TOOL_RESULTS_DIR = ".atom/tool-results"
...
def load_bundled_template(template_name: str) -> str | None:
    """Read a bundled template file from the atom package."""
    from importlib.resources import files as pkg_files

    with suppress(Exception):
        tpl = pkg_files("atom") / "templates" / template_name
        if tpl.is_file():
