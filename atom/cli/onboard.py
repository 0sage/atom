"""Interactive onboarding questionnaire for atom."""
...
from atom.cli.models import (
    format_token_count,
    get_model_context_limit,
    get_model_suggestions,
)
from atom.config.loader import get_config_path, load_config
...
def _show_main_menu_header() -> None:
    ...
    body.add_row(f"{__logo__} [bold {_UI_TEXT}]atom[/] [{_UI_MUTED}]v{__version__}[/]")
...
def _handle_fallback_models_field(
    ...
    from atom.config.schema import InlineFallbackConfig
...
def _handle_search_provider_field(
    ...
    from atom.agent.tools.web import SEARCH_PROVIDER_OPTIONS
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from atom.config.schema to get
...
def _quick_start_requires_base_url(provider_name: str, info: _QuickStartProviderInfo | None) -> bool:
    ...
    if info is None or info.default_api_base:
        return False
    return info.backend == "openai_compat" and (info.is_direct or info.is_local)
...
def _show_quick_start_summary(config: Config) -> None:
    ...
    model_label = f"{preset.provider}/{preset.model}" if preset else "not set"
    rows = [
        ("Status", status),
        ("Model", model_label),
    ]
    ...
    console.print()
    console.print(f"[{_UI_ACCENT}]Next steps[/]")
    console.print(f"  [{_UI_TEXT}]Chat from the terminal:[/] atom agent -m \"hello\"")
    console.print(f"  [{_UI_TEXT}]Or run it interactively:[/] atom agent")
    console.print(f"  [{_UI_MUTED}]Add a chat channel (Telegram, Slack, ...):[/] atom plugins list")
    console.print(f"  [{_UI_MUTED}]Then enable one:[/] atom plugins enable telegram")
    console.print(f"  [{_UI_MUTED}]Configure it:[/] atom onboard --wizard")
    console.print(f"  [{_UI_MUTED}]Run the channels:[/] atom gateway")
    console.print()


def _configure_quick_start(config: Config) -> bool:
    """First-run path: provider + credentials + model, with advanced settings hidden."""
    console.clear()
    _show_section_header(
        "Quick Start",
        "Choose provider endpoint, then add credentials and a model.",
    )
...
def run_quick_start_onboard(initial_config: Config) -> OnboardResult:
    """Run the compact provider + model setup path directly."""
    _get_questionary()
