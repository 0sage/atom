...
from atom.utils.helpers import sanitize_surrogates as _sanitize_surrogates
...
class SafeFileHistory(FileHistory):
    ...
    Special Unicode input (emoji, mixed-script) can produce surrogate
    characters that crash prompt_toolkit's file write. See issue #2846.
    """
...
def _init_prompt_session() -> None:
    ...
    from atom.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    if show_header:
        console.print()
        console.print(f"[cyan]{__logo__} atom[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        def _render(target: Console) -> None:
            target.print()
            target.print(f"[cyan]{__logo__} atom[/cyan]")
