...
def _status_command(config_path: Path) -> str:
    return f'atom status --config "{config_path}"'
...
def _print_model_setup_steps(config_path: Path) -> None:
    ...
    console.print(f"  Setup: run [cyan]atom onboard --wizard {escape(config_arg)}[/cyan]")
...
def _load_inspection_config(
    ...
    from atom.config.errors import ConfigLoadError
    from atom.config.loader import get_config_path, load_config, set_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from atom.config.paths import get_cron_dir
...
def _validate_gateway_startup(config: Config) -> str | None:
    """Validate gateway startup config, exiting with guidance when it cannot start."""
    from atom.config.loader import get_config_path
...
def _provider_setup_error(config: Config) -> str | None:
    ...
    from atom.providers.factory import validate_provider_setup
