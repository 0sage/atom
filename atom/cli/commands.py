...
# Remove default handler and re-add with unified atom format
logger.remove()
...
from atom import __logo__, __version__  # noqa: E402
from atom import optional_features as feature_support  # noqa: E402
from atom.agent.hooks import create_file_edit_activity_hook  # noqa: E402
from atom.agent.loop import AgentLoop  # noqa: E402
from atom.agent.tools.mcp import MCPProvider  # noqa: E402
from atom.agent.tools.registry import ToolRegistry  # noqa: E402
from atom.cli import terminal as cli_terminal  # noqa: E402
from atom.cli.agent import agent  # noqa: E402
from atom.cli.gateway import create_gateway_app  # noqa: E402
from atom.cli.gateway_runtime import _run_gateway  # noqa: E402
from atom.cli.log_control import _set_atom_logs  # noqa: E402
from atom.cli.runtime_config import (  # noqa: E402
...
from atom.config.paths import get_workspace_path  # noqa: E402
from atom.config.schema import Config  # noqa: E402
from atom.security.network import is_loopback_host  # noqa: E402
from atom.utils.helpers import sanitize_surrogates as _sanitize_surrogates  # noqa: E402,F401
from atom.utils.helpers import (  # noqa: E402
...
app = typer.Typer(
    ...
    help=f"{__logo__} atom - a self-hosted personal AI agent runtime",
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} atom v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """atom - a self-hosted personal AI agent runtime."""
    pass
...
def onboard(
    ...
    """Initialize atom configuration and workspace."""
    from atom.config.loader import get_config_path, load_config, save_config, set_config_path
    ...
    if wizard:
        from atom.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'atom onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    gateway_cmd = "atom gateway"
    if explicit_config:
        gateway_cmd += f' -c "{config_path}"'

    typer.echo(f"\n✓ atom is ready. Run: {gateway_cmd}")
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from atom.channels.contracts import channel_default_config
    from atom.channels.registry import discover_plugins
    from atom.config.loader import merge_missing_defaults
...
def trigger(
    ...
    from atom.triggers.local_store import (
        LocalTriggerStore,
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show atom runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: atom plugins enable api[/red]")
        raise typer.Exit(1)

    from atom.api.server import create_app
    from atom.bus.queue import MessageBus
    from atom.providers.image_generation import image_gen_provider_configs
    from atom.session.manager import SessionManager

    _set_atom_logs(verbose)
...
def channels_status(
    ...
    """Show channel status."""
    from atom.channels.registry import discover_all
...
plugins_app = typer.Typer(help="Manage optional atom features")
...
def plugins_list(
    ...
    """List optional atom features."""
    from atom.channels.registry import discover_plugins
    from atom.config.loader import load_config, set_config_path
...
def plugins_enable(
    name: str = typer.Argument(..., help="Feature name (e.g. api, langfuse, telegram)"),
    config_path: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show optional package install logs"),
):
    """Enable a atom feature."""
    from atom.config.loader import get_config_path, set_config_path
...
def plugins_disable(
    ...
    """Disable a atom channel feature."""
    from atom.config.loader import get_config_path, set_config_path
...
def status(
    ...
    console.print(f"{__logo__} atom Status\n")
    ...
    if config_path.exists():
        ...
        from atom.config.loader import resolve_config_env_vars, resolve_env_refs
        from atom.providers.registry import PROVIDERS
        ...
        if provider_ready:
            console.print()
            console.print('Next: [cyan]atom agent -m "Hello!"[/cyan]')
            console.print(
