...
from atom.agent.tools.mcp import MCPProvider
...
from atom.config.paths import is_default_workspace
from atom.config.schema import Config
from atom.security.network import is_loopback_host
from atom.session.keys import UNIFIED_SESSION_KEY, last_channel_from_metadata
from atom.utils.evaluator import evaluate_response, resolve_evaluator_prompt
from atom.utils.helpers import sync_workspace_templates
...
def _host_for_local_probe(host: str) -> str:
    """Map bind hosts to a host this process can actually connect to."""
    if host in {"0.0.0.0", ""}:
        return "127.0.0.1"
    if host == "::":
        return "[::1]"
...
def _gateway_health_url(host: str, port: int) -> str:
    ...
    return f"http://{_host_for_local_probe(host)}:{port}/health"
...
def _print_foreground_port_conflict(
    *,
    ...
    console.print(
        "[red]Error: atom cannot start because one of its local ports is already in use.[/red]"
    )
    console.print(f"  {label}: [cyan]{host}:{port}[/cyan]")
    console.print()
    console.print("If this is an existing atom instance, use it or stop it first:")
    console.print("  [cyan]atom gateway status[/cyan]")
    console.print("  [cyan]atom gateway stop[/cyan]")
    console.print("Or choose a different port with [cyan]--port[/cyan].")
...
def _run_gateway(
    ...
    *,
    ...
    """Shared foreground gateway runtime: agent loop, channels, cron, health endpoint."""
    from atom.agent.model_presets import load_model_preset_catalog
    ...
    from atom.config.watcher import watch_config_file
    from atom.cron.bound_runner import run_bound_cron_job
    from atom.cron.service import CronJobSkippedError, CronService
    from atom.cron.session_turns import is_bound_cron_job
    from atom.cron.types import CronJob
    from atom.providers.factory import (
        ProviderSnapshot,
    ...
    from atom.session.token_usage import TokenUsageHook
    from atom.triggers.local_runner import run_local_trigger_queue
    from atom.triggers.local_store import LocalTriggerStore

    port = port if port is not None else config.gateway.port
    gateway_host_for_probe = _host_for_local_probe(config.gateway.host)
    if health_server_enabled and _tcp_endpoint_reachable(gateway_host_for_probe, port):
        _print_foreground_port_conflict(
            host=gateway_host_for_probe,
            port=port,
            label="Gateway health",
        )
        raise typer.Exit(1)

    console.print(f"{__logo__} Starting atom gateway version {__version__} on port {port}...")
    ...
    def _load_gateway_provider_snapshot(
        ...
        **kwargs: Any,
        ...
        try:
            return load_provider_snapshot(*args, **kwargs)
    ...
    from atom.config.loader import get_config_path
    from atom.gateway.runtime import GatewayRuntime, GatewayRuntimePaths
    ...
    turn_delivery_factory = TurnDeliveryFactory(bus, runtime_events)
    ...
    from atom.session.keys import session_key_for_channel
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.name == "dream":
            from atom.agent.memory import DreamRunProgress, MemoryStore
            ...
            try:
                ...
                from atom.session.token_usage import record_response_token_usage
    ...
    from atom.cron.types import CronJob, CronPayload, CronSchedule
        ...
        try:
            ...
            tasks = [
                asyncio.create_task(
                    ...
                    name="atom-config-watcher",
                ),
                asyncio.create_task(_run_agent(), name="atom-agent-loop"),
                asyncio.create_task(channels.start_all(), name="atom-channels"),
                asyncio.create_task(
                    ...
                    name="atom-local-triggers",
                ),
            ]
            if health_server_enabled:
                tasks.append(asyncio.create_task(
                    _health_server(config.gateway.host, port),
                    name="atom-health-server",
                ))
            runtime_tasks = asyncio.gather(*tasks)
            shutdown_task = asyncio.create_task(
                shutdown_event.wait(),
                name="atom-gateway-shutdown",
            )
