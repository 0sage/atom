...
from atom.security.network import resolve_url_target
...
def _validate_generic(name: str, values: dict[str, Any]) -> dict[str, Any]:
    ...
    if spec is not None and spec.required:
        checks.append(_check("manual_review", "Manual setup", "skipped", "This channel can be checked from saved fields, but not fully verified here."))
    ...
    return _payload(name, "unsupported", [_check("support", "Setup check", "skipped", "This channel does not support automated setup checks yet.")])
...
# each package own its platform checks without depending on the management
