"""Optional atom feature discovery and enablement."""
...
from atom.channels.registry import channel_default_enabled
...
def optional_dependency_groups_from_metadata() -> dict[str, list[str] | None]:
    ...
    try:
        extras = metadata("atom").get_all("Provides-Extra") or []
        raw_requirements = requires("atom") or []
...
def install_args_for_extra(
    ...
    # atom is not published on PyPI, and the PyPI name `atom` is an unrelated
    # project, so a bare `atom[extra]` target would install the wrong package.
    target = f"atom[{extra}] @ git+https://github.com/0sage/atom.git"
...
def channel_enabled(
    ...
    *,
    ...
    if plugin is None:
        from atom.channels.registry import load_channel_plugin
    ...
    *,
    ...
    if name in _BUNDLED_FEATURE_ALIASES:
        payload = optional_features_payload(
            last_action={
                "ok": True,
                "message": f"Feature '{name}' is included with atom",
                "enabled": True,
            }
        )
