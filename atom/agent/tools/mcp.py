...
from atom.agent.tools.base import Tool, ToolResult
...
if TYPE_CHECKING:
    ...
    from atom.agent.tools.mcp_oauth import MCPOAuthHandlers
    from atom.config.schema import Config, MCPServerConfig
...
class MCPToolWrapper(_MCPWrapperBase):
    """Wraps a single MCP server tool as a atom Tool."""
    ...
    def _store_image_block(
        ...
        from atom.utils.artifacts import ArtifactError, store_generated_image_artifact
...
class MCPResourceWrapper(_MCPWrapperBase):
    """Wraps an MCP resource URI as a read-only atom Tool."""
...
class MCPPromptWrapper(_MCPWrapperBase):
    """Wraps an MCP prompt as a read-only atom Tool."""
        ...
        try:
            ...
            if cfg.auth == "oauth":
                if transport_type not in {"sse", "streamableHttp"}:
                    ...
                    return False
                from atom.agent.tools.mcp_oauth import (
...
def _configured_servers(config: Config) -> dict[str, MCPServerConfig]:
    from atom.agent.plugins import agent_plugin_mcp_servers
...
def _load_current_servers() -> dict[str, MCPServerConfig]:
    from atom.config.loader import load_config, resolve_config_env_vars
...
class MCPProvider:
    ...
    async def connect(self) -> None:
        ...
        async with self._lock:
            ...
            if oauth_servers:
                from atom.agent.tools.mcp_oauth import mcp_oauth_has_credentials
    ...
    async def reload(self) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            try:
                ...
                return {
                    "ok": False,
                    "message": "Could not reload MCP config. Restart atom to pick up changes.",
            ...
            if failed:
                ...
                message = "MCP connections refreshed without restarting atom."
            else:
                message = "MCP config reloaded without restarting atom."
