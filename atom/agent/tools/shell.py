...
from atom.agent.tools.sandbox import wrap_command
...
def _reap_pid(pid: int) -> None:
    ...
    Uses ``os`` capability checks rather than assuming ``waitpid`` exists so
    this stays safe on any platform where it is unavailable.
    """
...
@tool_parameters(
    tool_parameters_schema(
        ...
        shell=StringSchema(
            (
                "Override the Unix shell only when needed. Omit to use "
...
class ExecTool(Tool):
    ...
    def __init__(
        ...
        self.deny_patterns = (deny_patterns or []) + [
            ...
            r":\(\)\s*\{.*\};\s*:",          # fork bomb
            # Block writes to atom internal state files (#2989).
    ...
    def description(self) -> str:
        platform_note = (
            "On Unix, commands run through bash by default; pass shell='sh' "
    ...
    def _wrap_path_export(self, command: str, env: dict[str, str]) -> str:
        ...
        if self.path_prepend:
            env["ATOM_PATH_PREPEND"] = self.path_prepend
            segments.append("$ATOM_PATH_PREPEND")
        segments.append("$PATH")
        if self.path_append:
            env["ATOM_PATH_APPEND"] = self.path_append
            segments.append("$ATOM_PATH_APPEND")
    ...
    def _guard_command(
        ...
        *,
        ...
        from atom.security.network import contains_internal_url
    ...
    def _extract_absolute_paths(command: str) -> list[str]:
        ...
        home_paths = re.findall(r"(?:^|[\s>='\"])(~[/+][^\s\"'>;|<]*)", command) # home shortcut: ~/ or ~+
