...
from atom.agent import context as agent_context
from atom.agent import model_presets as preset_helpers
from atom.agent.autocompact import AutoCompact
from atom.agent.automation_turns import publish_next_deferred_turn
from atom.agent.context import ContextBuilder
from atom.agent.cron_turns import CronTurnCoordinator
from atom.agent.hook import AgentHook, AgentTurnHookFactory
from atom.agent.memory import Consolidator
from atom.agent.model_runtime import ModelRuntimeResolver
from atom.agent.runner import _MAX_INJECTIONS_PER_TURN, AgentRunner, AgentRunSpec
from atom.agent.subagent import SubagentManager
from atom.agent.tools.context import RequestContext, bind_request_context, reset_request_context
from atom.agent.tools.exec_session import ExecSessionManager
from atom.agent.tools.file_state import FileStateStore, bind_file_states, reset_file_states
from atom.agent.tools.message import MessageTool
from atom.agent.tools.registry import ToolRegistry
from atom.agent.tools.runtime_control import AgentRuntimeControl
from atom.agent.tools.self import MyTool
from atom.agent.turn_delivery import (
    TurnDelivery,
    TurnDeliveryFactory,
)
from atom.agent.turn_delivery import TurnRoute as TurnRoute
from atom.agent.turn_hooks import AgentTurnHookSpec, build_agent_turn_hook
from atom.bus.events import InboundMessage, OutboundMessage
from atom.bus.outbound_events import StreamedResponseEvent
from atom.bus.queue import MessageBus
from atom.bus.runtime_events import RuntimeEventBus
from atom.command import CommandContext, CommandRouter, register_builtin_commands
from atom.config.schema import AgentDefaults, ModelPresetConfig
from atom.providers.base import LLMProvider, ProviderConversationState
...
from atom.security.workspace_access import (
    WorkspaceScopeResolver,
    bind_workspace_scope,
    reset_workspace_scope,
)
from atom.session import turn_continuation
from atom.session.automation_turns import automation_history_overrides
from atom.session.history_visibility import HIDDEN_HISTORY_META
from atom.session.keys import UNIFIED_SESSION_KEY, remember_last_channel
from atom.session.manager import (
    Session,
    SessionManager,
    replay_max_messages_for_context,
)
from atom.session.model_selection import (
    SESSION_MODEL_PRESET_METADATA_KEY,
    model_preset_from_metadata,
)
from atom.triggers.local_turns import LocalTriggerTurnCoordinator
from atom.utils.cancellation import task_is_cancelling
from atom.utils.document import reference_non_image_attachments
from atom.utils.helpers import image_placeholder_text
from atom.utils.helpers import truncate_text as truncate_text_fn
from atom.utils.llm_runtime import LLMRuntime
from atom.utils.runtime import (
    EMPTY_FINAL_RESPONSE_MESSAGE,
)

if TYPE_CHECKING:
    from atom.config.schema import (
        ChannelsConfig,
        Config,
        ProviderConfig,
        ToolsConfig,
    )
    from atom.cron.service import CronService
...
class AgentLoop:
    ...
    def __init__(
        ...
        if (
            ...
            and "openai" not in self._image_generation_provider_configs
        ):
            self._image_generation_provider_configs["openai"] = image_generation_provider_config
        ...
        # ATOM_MAX_CONCURRENT_REQUESTS: <=0 means unlimited; default 3.
        _max = int(os.environ.get("ATOM_MAX_CONCURRENT_REQUESTS", "3"))
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from atom.providers.factory import make_provider
    ...
    async def _process_message(
        ...
        ctx = TurnContext(
            ...
            kind=kind,
            delivery=delivery,
            original_user_text=(None if kind is TurnKind.SYSTEM else msg.content),
    ...
    async def _dispatch_command(self, ctx: TurnContext) -> bool:
        ...
        if result is not None:
            ...
            # turn here so history readers after _turn_end see the
    ...
    async def _persist_turn(self, ctx: TurnContext) -> None:
        ...
        latency_started_at = (
            ...
            if ctx.kind is TurnKind.SYSTEM and ctx.visible_run_started_at is not None
