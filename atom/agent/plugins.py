...
def _load_manifest(plugin_root: Path) -> AgentPlugin | None:
    ...
    atom_value = extension_payload.get("dev.atom")
    atom = cast(dict[str, object], atom_value) if isinstance(atom_value, dict) else {}
    return AgentPlugin(
        name=name,
        root=plugin_root,
        description=_string(payload.get("description")),
        repository=_string(payload.get("repository")),
        display_name=_string(atom.get("displayName")) or name,
        category=_string(atom.get("category")) or "Plugin",
        accent_color=_accent_color(atom.get("accentColor")),
        logo=_plugin_logo(atom.get("logo"), plugin_root),
        permissions=_string_tuple(atom.get("permissions")),
    )
...
def _plugin_logo(value: object, plugin_root: Path) -> str | None:
    """Resolve atom's optional packaged logo extension."""
    if value is None:
        return None
