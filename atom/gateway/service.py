...
class GatewayServiceInstaller:
    ...
    def _install_launchd(
        ...
        *,
        ...
        stdout_path = self.home / ".atom" / "logs" / f"{log_stem}.launchd.log"
        stderr_path = self.home / ".atom" / "logs" / f"{log_stem}.launchd.err.log"
        payload = {
            "Label": label,
...
def _launchd_label(name: str) -> str:
    if name.startswith("ai.atom."):
        return name
    suffix = _safe_service_name(name).removeprefix("atom-").replace("-", ".")
    return f"ai.atom.{suffix}"
