...
Runtime state fan-out follows the same boundary. `AgentLoop` may publish generic runtime events from `atom.bus.runtime_events` (`SessionTurnStarted`, `TurnRunStatusChanged`, `TurnCompleted`, `SessionTurnPersisted`, `RuntimeModelChanged`) for turn/run/model state changes. Those events must stay transport-neutral: any wire framing, marker name, or presentation detail belongs in the consumer — a channel adapter, `atom/agent/turn_delivery.py`, or an SDK subscriber such as `atom/sdk/clients.py`. Do not add a consumer's vocabulary to the event payloads.
