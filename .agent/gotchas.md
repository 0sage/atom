...
atom targets Linux and macOS only. Do not add Windows-specific branches, PowerShell fallbacks, or `sys.platform == "win32"` handling.
- `cli/commands.py` forces `sys.stdout`/`stderr` to UTF-8 on startup to handle emoji and multilingual input.
- Still prefer `pathlib.Path` over string path munging.

## Prompt Templates

Agent system prompts and scenario-specific instructions live in `atom/templates/` as Jinja2 markdown files (`identity.md`, `platform_policy.md`, `HEARTBEAT.md`, `SOUL.md`, etc.). Changing these files alters agent behavior as directly as changing Python code. They are loaded by `utils/prompt_templates.py`.
...
Built-in skills live in `atom/skills/` (markdown + YAML frontmatter format). Agent capabilities that are "know-how" rather than code should be added as skills, not hardcoded into the agent loop. External skills can be published to and installed from ClawHub.
