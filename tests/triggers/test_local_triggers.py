...
from atom.session.metadata_keys import WEBUI_MESSAGE_SOURCE_METADATA_KEY, WEBUI_TURN_METADATA_KEY
from atom.triggers.local_runner import run_local_trigger_queue
from atom.triggers.local_store import LocalTriggerStore, TriggerDisabledError
...
async def test_local_trigger_queue_submits_bound_inbound_message(tmp_path: Path) -> None:
    ...
    # Stored origin metadata may predate the WebUI removal; stale turn identity is
    # dropped rather than copied into new turns.
    assert WEBUI_TURN_METADATA_KEY not in msg.metadata
