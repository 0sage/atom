...
async def test_openai_compat_disables_sdk_retries_by_default() -> None:
    with patch("atom.providers.openai_compat_provider.AsyncOpenAI") as mock_client:
...
def test_anthropic_normalizes_versioned_base_url() -> None:
    with patch("anthropic.AsyncAnthropic") as mock_client:
        AnthropicProvider(
            api_key="sk-test",
            api_base="https://proxy.internal/anthropic/v1",
            default_model="claude-sonnet-4-5",
        )

    kwargs = mock_client.call_args.kwargs
    assert kwargs["base_url"] == "https://proxy.internal/anthropic"
