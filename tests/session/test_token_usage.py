...
from atom.session.token_usage import (
    TokenUsageHook,
...
def test_payload_tolerates_malformed_persisted_day_keys(tmp_path, monkeypatch) -> None:
    ...
    """
    monkeypatch.setattr("atom.session.token_usage.get_webui_dir", lambda: tmp_path / "webui")
...
def test_record_response_token_usage_uses_response_usage(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr("atom.session.token_usage.get_webui_dir", lambda: tmp_path / "webui")
    monkeypatch.setattr("atom.session.token_usage._local_day", lambda *_, **__: "2026-06-03")
