...
async def test_exec_blocks_wget_localhost():
    tool = ExecTool()
    with patch("atom.security.network.socket.getaddrinfo", _fake_resolve_localhost):
...
# --- #2989: block writes to atom internal state files -----------------
...
def test_exec_blocks_format_command(command):
    """A standalone ``format`` disk command must be denied."""
    tool = ExecTool()
    result = tool._guard_command(command, "/tmp")
    assert result is not None
    assert "deny pattern filter" in result.lower()
...
def test_exec_allows_workspace_paths_from_subdirectory(tmp_path):
    ...
    commands in ``~/.atom/workspace/obsidian_notes`` reference paths
