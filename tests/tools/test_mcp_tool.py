...
import atom.agent.tools.mcp as mcp_mod
from atom.agent.tools.mcp import (
    MCPPromptWrapper,
    MCPProvider,
    MCPResourceWrapper,
    MCPToolWrapper,
...
from atom.agent.tools.registry import ToolRegistry, is_tool_error_result
...
async def test_saved_oauth_http_403_projects_failed_runtime_without_details(
    ...
    oauth_mod = ModuleType("atom.agent.tools.mcp_oauth")
    ...
    monkeypatch.setitem(sys.modules, "atom.agent.tools.mcp_oauth", oauth_mod)
...
async def test_connect_mcp_servers_enabled_tools_warns_on_unknown_entries(
    ...
    monkeypatch.setattr("atom.agent.tools.mcp.logger.warning", _warning)
...
async def test_connect_mcp_servers_passes_stdio_cwd(
    ...
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake", cwd="/tmp/atom-mcp-test")},
        registry,
    )
    for stack in stacks.values():
        await stack.aclose()

    assert captured["cwd"] == "/tmp/atom-mcp-test"
