...
from atom.agent.tools.web import WebFetchTool, _validate_url
...
def _patched_web_fetch():
    with (
        patch("atom.security.network.socket.getaddrinfo", _fake_resolve_public),
        patch("atom.agent.tools.web.httpx.AsyncClient", FakeClient),
        patch("atom.agent.tools.web._pinned_dns_transport", lambda: object()),
    ):
        yield
