...
def test_load_config_reports_malformed_environment_safely(
    ...
    monkeypatch.setenv("ATOM_PROVIDERS", invalid_value)
    ...
    assert "complex ATOM_* values use valid JSON" in str(error)
...
def test_load_config_error_does_not_trust_custom_validator_message(tmp_path) -> None:
    ...
    config_path.write_text(
        json.dumps({"providers": {"groq": {"thinkingStyle": secret}}}),
        encoding="utf-8",
    )
    ...
    message = str(exc_info.value)
    assert "providers.groq.thinkingStyle" in message
