...
from atom.channels.registry import discover_plugins, load_channel_plugin
...
def _multi_instance_specs(
    ...
    *,
    ...
    return [
        ChannelInstanceSpec(instance["id"], dict(instance))
        for instance in instances
        if not enabled_only or instance.get("enabled", True)
    ]


def _multi_update_instance_config(
    section: Any,
    values: dict[str, Any],
    *,
    ...
    instances = [dict(item) for item in existing.get("instances", [])]
    merged = dict(values)
    merged["id"] = instance_id
    for index, instance in enumerate(instances):
        if instance.get("id") == instance_id:
            instances[index] = instance | merged
            break
    else:
        instances.append(merged)
    return existing | {"instances": instances}


_MULTI_PLUGIN = ChannelPlugin(
    name="multi_contract",
    display_name="Multi contract",
    runtime=f"{__name__}:_SingleChannel",
    ...
    management=ChannelManagementSpec(
        ...
        update_instance_config=_multi_update_instance_config,
...
def _identity_instance_specs(
    section: Any,
    *,
    enabled_only: bool = True,
) -> list[ChannelInstanceSpec]:
    """Multi-instance expansion that collapses duplicate credential identities.

    Mirrors the contract a credential-scoped channel needs: two instances that
    share one app identity cannot both hold a runtime connection, so only the
    first enabled owner of an identity is expanded.
    """
    instances = section.get("instances", []) if isinstance(section, dict) else []
    specs: list[ChannelInstanceSpec] = []
    seen_identities: set[str] = set()
    for instance in instances:
        if enabled_only and not instance.get("enabled", True):
            continue
        identity = str(instance.get("appId", ""))
        if enabled_only:
            if identity in seen_identities:
                continue
            seen_identities.add(identity)
        specs.append(ChannelInstanceSpec(instance["id"], dict(instance)))
    return specs


_IDENTITY_PLUGIN = ChannelPlugin(
    name="identity_contract",
    display_name="Identity contract",
    runtime=f"{__name__}:_SingleChannel",
    setup=ChannelSetupSpec(
        fields={
        ...
        required=(SetupRequirement.field("appId"), SetupRequirement.field("appSecret")),
    ),
    management=ChannelManagementSpec(
        multi_instance=True,
...
def test_multi_instance_support_is_declared_by_management_spec() -> None:
    ...
    assert _MULTI_PLUGIN.management.multi_instance is True
    assert load_channel_plugin("telegram").management.multi_instance is False
...
def test_settings_contract_import_does_not_eagerly_load_runtime_graph() -> None:
    code = """
import sys
import atom.channels.validation

unexpected = {
    "atom.channels.manager",
} & sys.modules.keys()
            ...
            _MULTI_PLUGIN,
            {
                "instances": [
                    ...
                    },
                    {
                        "id": "product",
                        "enabled": True,
                        "token": "secret-product",
                    },
                ]
            },
            "product",
            {"default", "product"},
            id="synthetic-multi-instance",
        ),
    ]
...
def test_instance_contract_skips_duplicate_app_identity() -> None:
    section = {
        "instances": [
            {
                "id": "default",
    ...
    specs = channel_instance_specs(_IDENTITY_PLUGIN, section)

    assert [spec.instance_id for spec in specs] == ["default"]


def test_feature_state_matches_runtime_duplicate_filter() -> None:
    section = {
        "instances": [
            {
                "id": "default",
    ...
    instances = channel_feature_instances(
        _IDENTITY_PLUGIN,
        section,
        setup_spec=_IDENTITY_PLUGIN.setup,
    )
...
def test_runtime_duplicate_ignores_disabled_identity_owner() -> None:
    section = {
        "instances": [
            {
                "id": "default",
...
def test_instance_write_preserves_duplicate_app_identity() -> None:
    section = {
        "instances": [
            {
                "id": "default",
