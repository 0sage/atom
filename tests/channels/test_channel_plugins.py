...
from atom.channels.contracts import (
    ChannelFieldSpec,
    ChannelInstanceSpec,
...
from atom.channels.manager import ChannelManager
from atom.channels.plugin import ChannelPlugin, load_channel_package
from atom.config.loader import save_config
from atom.config.schema import ChannelsConfig, Config
from atom.providers.transcription import GroqTranscriptionProvider as _GroqProvider
from atom.providers.transcription import OpenAITranscriptionProvider as _OpenAIProvider
from atom.utils.restart import RestartNotice
...
def _stub_channel_registry(
    ...
    *plugins: ChannelPlugin,
    ...
    monkeypatch.setattr("atom.channels.registry.discover_plugins", discover)
...
def _stub_optional_feature_cli(
    ...
    *,
    ...
    monkeypatch.setattr("atom.optional_features.optional_dependency_groups", lambda: extras)
    monkeypatch.setattr("atom.optional_features.extra_installed", lambda _name, _deps: installed)
    if commands is not None:
        monkeypatch.setattr(
            "atom.optional_features.run_install_command",
...
def test_special_setup_validation_is_owned_by_channel_package(name: str):
    ...
    assert plugin.setup.validator.__module__ == f"atom.channels.{name}.validation"


def test_descriptor_defaults_cover_onboarding_fields_without_runtime_import():
    """Manifest field defaults are readable without importing the runtime."""

    class _DefaultsChannel(_FakePlugin):
        name = "defaults"
    ...
    plugin = _channel_plugin(
        ...
        setup=ChannelSetupSpec(
            fields={
                "msgFormat": ChannelFieldSpec(
                    kind="enum",
                    choices=frozenset({"plain", "markdown"}),
                    default="plain",
                ),
                "imapPort": ChannelFieldSpec(kind="int", default=993),
                "smtpPort": ChannelFieldSpec(kind="int", default=587),
            },
        ),
    )

    assert f"atom.channels.{plugin.name}.runtime" not in sys.modules
    assert channel_default_config(plugin)["msgFormat"] == "plain"
    assert channel_default_config(plugin)["imapPort"] == 993
    assert channel_default_config(plugin)["smtpPort"] == 587
...
def test_channel_manager_loads_descriptor_but_not_disabled_runtime(monkeypatch):
    ...
    monkeypatch.setattr(
        "atom.channels.registry._channel_package_names",
        lambda: ["fakeplugin"],
    )
    monkeypatch.setattr(
        "atom.channels.registry.load_channel_package",
...
def test_feature_payload_uses_unified_instance_activation(monkeypatch):
    from atom.optional_features import optional_features_payload
    ...
    _stub_channel_registry(monkeypatch, _channel_plugin(_FakeMultiChannel))
    monkeypatch.setattr("atom.optional_features.optional_dependency_groups", lambda: {})
...
def test_discover_plugins_loads_package_descriptors():
    ...
    with (
        patch("atom.channels.registry._channel_package_names", return_value=["line"]),
        patch("atom.channels.registry.load_channel_package", return_value=plugin),
    ):
...
def test_generic_plugin_validation_enforces_composite_requirements(
    ...
    from atom.channels.validation import validate_channel_config
    from atom.config import loader
...
def test_discover_plugins_skips_names_outside_enabled_set():
    ...
    with (
        patch("atom.channels.registry._channel_package_names", return_value=["disabled"]),
        patch("atom.channels.registry.load_channel_package", side_effect=_load_disabled),
    ):
...
def test_discover_plugins_handles_load_error():
    ...
    with (
        patch("atom.channels.registry._channel_package_names", return_value=["broken"]),
        patch("atom.channels.registry.load_channel_package", side_effect=_boom),
    ):
        result = discover_plugins()
...
def test_discover_all_includes_available_channel_packages():
    from atom.channels.registry import discover_all, discover_plugins
...
def test_discover_enabled_imports_only_enabled_packages():
    from atom.channels.registry import discover_enabled
...
def test_discover_enabled_warns_for_enabled_package_import_errors():
    ...
    plugin = ChannelPlugin(
        name="absent",
        display_name="Absent",
        runtime="missing.absent.runtime:AbsentChannel",
    )
    with patch("atom.channels.registry.logger.warning") as warning:
        result = discover_enabled(
            {"absent"},
            _plugins={"absent": plugin},
    ...
    assert warning.call_args.args[1] == "absent"
...
def test_manager_installs_manifest_dependencies_before_loading_enabled_channel(monkeypatch):
    from atom.optional_features import InstallResult
    ...
    monkeypatch.setattr("atom.optional_features.extra_installed", extra_installed)
    monkeypatch.setattr("atom.optional_features.install_extra", install_extra)
    config = Config.model_validate({
        "channels": {
...
def test_manager_reports_dependency_install_failure_as_runtime_failure(monkeypatch):
    ...
    _stub_channel_registry(monkeypatch, plugin)
    monkeypatch.setattr(
        "atom.optional_features.extra_installed",
        lambda _name, _dependencies: False,
    )
    monkeypatch.setattr(
        "atom.optional_features.install_extra",
...
def test_manager_loads_default_enabled_channel_from_default_config():
    from atom.channels.manager import ChannelManager

    class _FakeDefaultOn(_FakePlugin):
        name = "defaulton"
    ...
    plugin = _channel_plugin(_FakeDefaultOn, default_enabled=True)
    with patch("atom.channels.registry.discover_plugins", return_value={"defaulton": plugin}):
        mgr = ChannelManager(Config(), MessageBus())

    assert "defaulton" in mgr.channels
    assert mgr.channels["defaulton"].config["enabled"] is True
    assert mgr.channels["defaulton"].config["host"] == "127.0.0.1"


def test_manager_respects_explicitly_disabled_channel_config():
    from atom.channels.manager import ChannelManager

    config = Config.model_validate({"channels": {"defaulton": {"enabled": False}}})
    plugin = ChannelPlugin(
        name="defaulton",
        display_name="Default On",
        runtime="missing.defaulton.runtime:DefaultOnChannel",
        default_enabled=True,
    )
    with patch("atom.channels.registry.discover_plugins", return_value={"defaulton": plugin}):
        mgr = ChannelManager(config, MessageBus())

    assert "defaulton" not in mgr.channels
...
async def test_base_channel_reads_current_transcription_config_each_call(
    ...
    from atom.providers import transcription as transcription_mod
    ...
    save_config(config, config_path)
    monkeypatch.setattr("atom.config.loader._current_config_path", config_path)
...
async def test_base_channel_respects_disabled_transcription_config(
    ...
    with patch("atom.providers.transcription.GroqTranscriptionProvider") as provider:
...
def test_openai_transcription_provider_honors_api_base_argument():
    from atom.providers.transcription import OpenAITranscriptionProvider
...
async def test_transcription_provider_includes_language(tmp_path, provider_cls, language):
    ...
    with patch("atom.providers.transcription.httpx.AsyncClient", return_value=_stub_async_client(captured)):
...
def test_channels_login_uses_discovered_plugin_class(monkeypatch):
    ...
    monkeypatch.setattr("atom.config.loader.load_config", lambda config_path=None: Config())
    monkeypatch.setattr(
        "atom.channels.registry.discover_all",
...
def test_plugins_list_shows_available_features(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: {"telegram": ["python-telegram-bot>=22.6"], "langfuse": ["langfuse>=3.0.0,<4.0.0"]},
    )
...
def test_plugins_enable_channel_installs_extra_and_writes_config(monkeypatch, tmp_path):
    ...
    class _ExtraChannel(_FakePlugin):
        name = "extrachan"
        display_name = "Extra chan"
    ...
    config_path.write_text(
        json.dumps({"channels": {"extrachan": {"enabled": False, "token": "keep"}}}),
        encoding="utf-8",
    )
    ...
    _stub_optional_feature_cli(
        monkeypatch,
        extras={"extrachan": ["qrcode[pil]>=8.0", "pycryptodome>=3.20.0"]},
        installed=False,
        commands=commands,
        channels=["extrachan"],
        channel_cls=_ExtraChannel,
    )

    result = runner.invoke(app, ["plugins", "enable", "extrachan", "--config", str(config_path)])
    ...
    assert data["channels"]["extrachan"]["enabled"] is True
    assert data["channels"]["extrachan"]["token"] == "keep"
    assert data["channels"]["extrachan"]["allowFrom"] == []
...
def test_plugins_enable_extra_without_channel_only_installs(monkeypatch, tmp_path):
    ...
    log_flags: list[bool] = []
    config_path = tmp_path / "config.json"
    original_set_logs = cli_commands._set_atom_logs
    ...
    monkeypatch.setattr("atom.cli.commands._set_atom_logs", _set_logs)
...
def test_plugins_enable_logs_option_enables_atom_logs(monkeypatch, tmp_path):
    from typer.testing import CliRunner
    ...
    result = runner.invoke(
        app,
        ["plugins", "enable", "langfuse", "--logs", "--config", str(config_path)],
    )
...
def test_plugins_disable_rejects_non_channel_and_allows_channel(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "atom.optional_features.optional_dependency_groups",
        lambda: {"langfuse": ["langfuse>=3.0.0,<4.0.0"]},
    )

    non_channel = runner.invoke(
        app,
        ["plugins", "disable", "langfuse", "--config", str(config_path)],
    )
    channel = runner.invoke(
        app,
        ["plugins", "disable", "telegram", "--config", str(config_path)],
    )

    assert non_channel.exit_code == 1
    assert "Feature 'langfuse' cannot be disabled" in non_channel.output
    assert channel.exit_code == 0
    assert "Disabled channel 'telegram'" in channel.output
    assert json.loads(config_path.read_text(encoding="utf-8"))["channels"]["telegram"][
        "enabled"
    ] is False
...
def test_enable_optional_feature_skips_install_when_dependency_present(
    ...
    from atom.optional_features import InstallResult, enable_optional_feature
    ...
    monkeypatch.setattr("atom.optional_features.extra_installed", lambda _name, _deps: True)
    ...
    monkeypatch.setattr("atom.optional_features.install_extra", _install_extra)

    payload = enable_optional_feature("langfuse", config_path=config_path)

    assert install_calls == []
    assert payload["last_action"]["message"] == "Enabled feature 'langfuse'"
    assert payload["requires_restart"] is True
    assert not config_path.exists()


def test_enable_optional_feature_lazy_reader_does_not_require_restart(monkeypatch, tmp_path):
    from atom.optional_features import enable_optional_feature
    ...
    assert payload["last_action"]["message"] == "Feature 'documents' is included with atom"


def test_enable_optional_feature_reports_install_failure(monkeypatch, tmp_path):
    from atom.optional_features import (
        InstallResult,
    ...
    monkeypatch.setattr("atom.optional_features.extra_installed", lambda _name, _deps: False)
    monkeypatch.setattr(
        "atom.optional_features.install_extra",
        lambda _name, _deps, *, runner: InstallResult(
            False,
            "langfuse support",
            ["python", "-m", "pip", "install", "langfuse>=3.0.0,<4.0.0"],
            failed_cmd=["python", "-m", "pip", "install", "langfuse>=3.0.0,<4.0.0"],
            output="network unavailable",
        ),
    )

    with pytest.raises(OptionalFeatureError) as exc:
        enable_optional_feature("langfuse", config_path=config_path)
...
def test_disable_optional_feature_rejects_unknown_features_and_non_channels(
    ...
    from atom.optional_features import OptionalFeatureError, disable_optional_feature
    ...
    with pytest.raises(OptionalFeatureError) as non_channel:
        disable_optional_feature("langfuse", config_path=config_path)
    assert non_channel.value.status == 400
    assert non_channel.value.message == "Feature 'langfuse' cannot be disabled"

    assert not config_path.exists()


def test_disable_optional_feature_writes_channel_disabled(monkeypatch, tmp_path):
    from atom.optional_features import disable_optional_feature
...
def test_enable_rejects_duplicate_instance_ids_without_writing(tmp_path):
    from atom.optional_features import OptionalFeatureError, set_channel_config_enabled
    ...
    config_path.write_text(
        json.dumps({
            "channels": {
                "managedmulti": {
                    "instances": [
                        {"id": "default", "enabled": False, "token": "A"},
                        {"id": "default", "enabled": True, "token": "B"},
                    ]
                }
            }
        }),
    ...
    with pytest.raises(OptionalFeatureError, match="duplicate instance id 'default'"):
        set_channel_config_enabled(config_path, "managedmulti", plugin, True)
...
def test_optional_features_payload_counts_enabled_channel_with_missing_dependency(
    ...
    feature = payload["features"][0]
    assert feature["name"] == "telegram"
    assert feature["enabled"] is True
    assert feature["installed"] is False
    assert feature["ready"] is False
    assert payload["enabled_count"] == 1


def test_live_runtime_status_overrides_enabled_configuration_for_webui():
    from atom.optional_features import with_channel_runtime_status
...
def test_package_manifest_metadata_drives_optional_feature_payload(monkeypatch):
    ...
    monkeypatch.setattr("atom.optional_features.extra_installed", record_extra)
...
def _local_state_plugin(default_state_dir: Path | None = None) -> ChannelPlugin:
    """A channel whose credentials live in a local login-state file, not config."""

    def local_state_present(section) -> bool:
        raw = (
            section.get("stateDir")
            if isinstance(section, dict)
            else getattr(section, "state_dir", None)
        )
        state_dir = Path(raw) if raw else default_state_dir
        return bool(state_dir and (state_dir / "account.json").is_file())

    return ChannelPlugin(
        name="localstate",
        display_name="Local state",
        runtime="missing.localstate.runtime:LocalStateChannel",
        setup=ChannelSetupSpec(
            fields={"stateDir": ChannelFieldSpec(kind="string")},
...
def test_optional_features_payload_detects_saved_login_state(tmp_path, monkeypatch):
    from atom.optional_features import optional_features_payload

    state_dir = tmp_path / "localstate-state"
    state_dir.mkdir()
    (state_dir / "account.json").write_text(
        json.dumps({"token": "saved-token"}),
        encoding="utf-8",
    )
    config = Config.model_validate({
        "channels": {
    ...
    _stub_channel_registry(monkeypatch, _local_state_plugin())
    monkeypatch.setattr("atom.optional_features.optional_dependency_groups", lambda: {})
...
def test_optional_features_payload_detects_legacy_default_state(tmp_path, monkeypatch):
    from atom.config import loader
    from atom.optional_features import optional_features_payload
    ...
    monkeypatch.setattr(loader, "_current_config_path", config_path)
    state_dir = tmp_path / "localstate"
    state_dir.mkdir()
    (state_dir / "account.json").write_text(
        json.dumps({"token": "legacy-token"}),
        encoding="utf-8",
    )
    _stub_channel_registry(monkeypatch, _local_state_plugin(state_dir))
    monkeypatch.setattr("atom.optional_features.optional_dependency_groups", lambda: {})
...
def test_optional_features_payload_requires_device_id_for_token_login(
    monkeypatch,
    device_id,
):
    from atom.optional_features import optional_features_payload
    ...
    plugin = ChannelPlugin(
        name="tokenlogin",
        display_name="Token login",
        runtime="missing.tokenlogin.runtime:TokenLoginChannel",
        setup=ChannelSetupSpec(
            fields={
                "homeserver": ChannelFieldSpec(kind="string"),
                "userId": ChannelFieldSpec(kind="string"),
                "password": ChannelFieldSpec(kind="secret"),
                "accessToken": ChannelFieldSpec(kind="secret"),
                "deviceId": ChannelFieldSpec(kind="string"),
            },
            required=(
                SetupRequirement.field("homeserver"),
                SetupRequirement.field("userId"),
                SetupRequirement.one_of(("password",), ("accessToken", "deviceId")),
            ),
        ),
    )
    config = Config.model_validate({
...
def _identity_channel_module():
    """A stand-in runtime module whose identity fetch can be monkeypatched."""
    import types

    module = types.ModuleType("tests_identity_channel_runtime")

    def fetch_app_identity(*_args):
        raise AssertionError("identity fetch must be patched by the test")

    module.SDK_AVAILABLE = False
    module.fetch_app_identity = fetch_app_identity
    ...
    class IdentityChannel(BaseChannel):
        ...
        @classmethod
        def refresh_feature_metadata(cls, config_path, *, instance_id="default") -> bool:
            """Persist freshly fetched display metadata onto the target instance."""
            if not module.SDK_AVAILABLE:
                return False
            data = json.loads(Path(config_path).read_text(encoding="utf-8"))
            section = data.setdefault("channels", {}).setdefault("identity", {})
            ...
            for instance in instances:
                if instance.get("id") != instance_id:
                    continue
                identity = module.fetch_app_identity(instance.get("appId"))
                instance.update(identity)
    ...
    module.IdentityChannel = IdentityChannel
    return module


_IDENTITY_MODULE = _identity_channel_module()
sys.modules["tests_identity_channel_runtime"] = _IDENTITY_MODULE


def _identity_instance_specs(section, *, enabled_only=True):
    """Expand flat legacy config or an explicit instance list, like a real channel."""
    values = section if isinstance(section, dict) else {}
    raw = values.get("instances")
    if not isinstance(raw, list) or not raw:
        legacy = {key: value for key, value in values.items() if key != "instances"}
        legacy.setdefault("id", "default")
        raw = [legacy]
    return [
        ChannelInstanceSpec(instance_id=item.get("id", "default"), config=item)
        for item in raw
        if not enabled_only or item.get("enabled", False)
    ]


def _identity_plugin() -> ChannelPlugin:
    return ChannelPlugin(
        name="identity",
        display_name="Identity",
        runtime="tests_identity_channel_runtime:IdentityChannel",
        setup=ChannelSetupSpec(
            fields={
                "appId": ChannelFieldSpec(kind="string"),
                "appSecret": ChannelFieldSpec(kind="secret"),
                "domain": ChannelFieldSpec(
                    kind="enum",
                    choices=frozenset({"primary", "secondary"}),
                    default="primary",
                ),
                "groupPolicy": ChannelFieldSpec(
                    kind="enum",
                    choices=frozenset({"mention", "open", "allowlist"}),
                    default="mention",
                ),
                "topicIsolation": ChannelFieldSpec(kind="bool", default=True),
            },
            required=(
        ...
        management=ChannelManagementSpec(
            multi_instance=True,
            instance_specs=_identity_instance_specs,
            update_instance_config=_fake_multi_update,
            runtime_name=lambda name, instance_id: (
                name if instance_id == "default" else f"{name}.{instance_id}"
            ),
...
def test_optional_features_payload_marks_disabled_channel_as_configured(monkeypatch):
    from atom.optional_features import optional_features_payload
    ...
    _stub_channel_registry(
        monkeypatch,
        replace(_identity_plugin(), runtime="missing.identity.runtime:IdentityChannel"),
    )
    monkeypatch.setattr("atom.optional_features.optional_dependency_groups", lambda: {})
    ...
    feature = payload["features"][0]
    assert feature["name"] == "identity"
    assert feature["enabled"] is False
    assert feature["configured"] is True
    assert feature["ready"] is False
    assert feature["setup"]["fields"][0]["key"] == "channels.identity.appId"
    assert payload["enabled_count"] == 0


def test_optional_features_payload_lists_channel_instances(monkeypatch):
    from atom.optional_features import optional_features_payload
    ...
    assert feature["instances"] == [
        {
            ...
            "configured": True,
            "config_values": {
                "channels.identity.appId": "cli_default",
                "channels.identity.domain": "primary",
                "channels.identity.groupPolicy": "mention",
                "channels.identity.topicIsolation": "true",
            },
            "configured_fields": [
                "channels.identity.appId",
                "channels.identity.appSecret",
                "channels.identity.domain",
                "channels.identity.groupPolicy",
                "channels.identity.topicIsolation",
            ],
        },
        {
            "id": "product",
            ...
            "configured": True,
            "config_values": {
                "channels.identity.appId": "cli_product",
                "channels.identity.domain": "primary",
                "channels.identity.groupPolicy": "mention",
                "channels.identity.topicIsolation": "true",
            },
...
def test_optional_features_payload_does_not_refresh_saved_identity(monkeypatch, tmp_path):
    from atom.config import loader
    from atom.optional_features import optional_features_payload
    ...
    monkeypatch.setattr(loader, "_current_config_path", config_path)
    _stub_channel_registry(monkeypatch, _identity_plugin())
    monkeypatch.setattr("atom.optional_features.optional_dependency_groups", lambda: {})
    monkeypatch.setattr(
        _IDENTITY_MODULE,
        "fetch_app_identity",
        lambda *_args: pytest.fail("feature discovery must not call the platform"),
    )
    before = config_path.read_text(encoding="utf-8")
    ...
    assert instance["display_name"] == "atom"
    assert instance["avatar_url"] == ""
    assert config_path.read_text(encoding="utf-8") == before


def test_enable_optional_feature_refreshes_identity(monkeypatch, tmp_path):
    from atom.config import loader
    from atom.optional_features import enable_optional_feature
    ...
    _stub_channel_registry(monkeypatch, _identity_plugin())
    monkeypatch.setattr("atom.optional_features.optional_dependency_groups", lambda: {})
    monkeypatch.setattr(_IDENTITY_MODULE, "SDK_AVAILABLE", True)
    monkeypatch.setattr(
        _IDENTITY_MODULE,
        "fetch_app_identity",
        lambda *_args: {
    ...
    payload = enable_optional_feature("identity", config_path=config_path)
    ...
    data = json.loads(config_path.read_text(encoding="utf-8"))
    saved = data["channels"]["identity"]["instances"][0]
...
def test_optional_features_payload_preserves_legacy_flat_config(monkeypatch, tmp_path):
    from atom.config import loader
    from atom.optional_features import optional_features_payload
    ...
    assert payload["features"][0]["instances"][0]["display_name"] == "default"
    assert config_path.read_text(encoding="utf-8") == before
    saved = json.loads(config_path.read_text(encoding="utf-8"))["channels"]["identity"]
...
def test_enable_uses_uv_when_tool_environment_has_no_pip(
    ...
    from atom import optional_features
    ...
    assert optional_features.install_extra("langfuse", ["langfuse>=3.0.0,<4.0.0"], runner=_run).ok is True
    assert calls == [
        [sys.executable, "-m", "pip", "install", "langfuse<4.0.0,>=3.0.0"],
        [
            "uv",
            "pip",
            "install",
...
def test_enable_bootstraps_pip_with_ensurepip(monkeypatch):
    ...
    assert optional_features.install_extra("langfuse", None, runner=_run).ok is True
    assert calls == [
        [sys.executable, "-m", "pip", "install", "atom[langfuse] @ git+https://github.com/0sage/atom.git"],
...
def test_install_extra_logs_command_and_output(monkeypatch):
    ...
    result = optional_features.install_extra("langfuse", ["langfuse>=3.0.0,<4.0.0"], runner=_run)

    assert result.ok is True
    assert any("Installing optional feature 'langfuse':" in record for record in records)
    assert any("Optional feature 'langfuse' install exited with code 0" in record for record in records)
...
def test_optional_dependency_metadata_for_enable():
    ...
    assert "langfuse>=3.0.0,<4.0.0" not in data["project"]["dependencies"]
    ...
    for name, expected in expected_channel_dependencies.items():
        plugin = load_channel_package(name)
        assert plugin is not None
        assert [dep.split(";")[0].strip() for dep in plugin.dependencies] == list(expected)
...
def test_optional_dependency_groups_falls_back_to_package_metadata(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda _name: [
            "packaging>=24.0",
            "langfuse>=3.0.0,<4.0.0; extra == 'langfuse'",
    ...
    assert deps == {"langfuse": ["langfuse>=3.0.0,<4.0.0; extra == 'langfuse'"]}
    assert optional_features.install_args_for_extra("langfuse", deps["langfuse"]) == (
        ["langfuse<4.0.0,>=3.0.0"],
        "langfuse support",
    )
...
def test_install_args_for_extra_resolves_metadata_markers_for_current_platform():
    ...
    deps = [
        f"current-platform-package>=1.0; sys_platform == '{current_platform}' and extra == 'langfuse'",
        "other-platform-package>=1.0; sys_platform == 'never' and extra == 'langfuse'",
    ]

    assert optional_features.install_args_for_extra("langfuse", deps) == (
...
def test_channel_default_config():
    ...
    from atom.channels.telegram.runtime import TelegramChannel
...
def test_channel_init_from_dict():
    ...
    from atom.channels.telegram.runtime import TelegramChannel
    bus = MessageBus()
    ch = TelegramChannel({"enabled": False, "token": "test-token", "allowFrom": ["*"]}, bus)
    assert ch.config.token == "test-token"
...
async def test_send_with_retry_retries_on_failure():
    ...
    with patch("atom.channels.manager.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_no_retry_when_max_is_zero():
    ...
    with patch("atom.channels.manager.asyncio.sleep", new_callable=AsyncMock):
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_propagates_cancelled_error_during_sleep():
    ...
    with patch("atom.channels.manager.asyncio.sleep", side_effect=cancel_during_sleep):
        with pytest.raises(asyncio.CancelledError):
            await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_notify_restart_done_waits_until_channel_starts():
    ...
    with patch("atom.channels.manager.consume_restart_notice_from_env", return_value=notice):
...
async def test_restart_notice_retries_until_running_channel_accepts_delivery():
    ...
    with patch("atom.channels.manager._SEND_RETRY_DELAYS", (0,)):
