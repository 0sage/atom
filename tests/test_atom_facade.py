"""Tests for the Atom programmatic facade."""
...
from atom.session.manager import FILE_MAX_MESSAGES
from atom.utils.llm_runtime import runtime_from_provider_snapshot
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        Atom.from_config("/nonexistent/config.json")
...
def test_from_config_missing_env_reports_explicit_config_path(
    ...
    name = "ATOM_TEST_SDK_MISSING_KEY"
    monkeypatch.delenv(name, raising=False)
    config_path = tmp_path / "custom.json"
    config_path.write_text(
        json.dumps({"providers": {"openai": {"apiKey": f"${{{name}}}"}}}),
        encoding="utf-8",
    )

    with pytest.raises(ConfigLoadError) as exc_info:
        Atom.from_config(config_path)
...
def test_from_config_creates_instance(tmp_path):
    ...
    bot = Atom.from_config(config_path, workspace=tmp_path)
...
def test_from_config_accepts_default_model_preset(tmp_path):
    ...
    bot = Atom.from_config(config_path, workspace=tmp_path, model_preset="fast")
...
def test_from_config_default_path():
    ...
    with patch("atom.config.loader.load_config") as mock_load, \
         patch("atom.providers.factory.make_provider") as mock_prov:
...
async def test_run_with_hooks(tmp_path):
    from atom.agent.hook import AgentHook, AgentHookContext, SDKCaptureHook
    from atom.bus.events import OutboundMessage
...
async def test_run_hooks_restored_on_error(tmp_path):
    ...
    from atom.agent.hook import AgentHook
...
def test_workspace_override(tmp_path):
    ...
    bot = Atom.from_config(config_path, workspace=custom_ws)
...
async def test_run_exposes_attributes_to_context_provider_without_persisting_them(tmp_path):
    ...
    from atom.providers.base import LLMResponse
...
async def test_persisted_turn_callback_is_best_effort_and_reads_display_safe_session(tmp_path):
    from atom import SessionTurnPersisted
    from atom.agent.loop import AgentLoop
    from atom.bus.queue import MessageBus
    from atom.providers.base import LLMResponse
...
def test_runtime_client_does_not_expose_generic_event_subscription():
    from atom.sdk.clients import RuntimeClient
...
def test_import_from_top_level():
    ...
    assert atom.Atom is Atom
    assert atom.RequestContext.__name__ == "RequestContext"
    assert atom.RuntimeContextBlock.__name__ == "RuntimeContextBlock"
    assert atom.RuntimeContextProvider is not None
    assert atom.SessionTurnPersisted.__name__ == "SessionTurnPersisted"
    assert atom.RunResult is RunResult
    assert atom.RunStream is RunStream
    assert atom.SessionInfo is SessionInfo
    assert atom.SessionSnapshot is SessionSnapshot
    assert atom.StreamEvent is StreamEvent
    assert atom.StreamEventType is StreamEventType
    assert atom.STREAM_EVENT_TEXT_DELTA == STREAM_EVENT_TEXT_DELTA
    assert atom.STREAM_EVENT_RUN_COMPLETED == STREAM_EVENT_RUN_COMPLETED
    assert atom.STREAM_EVENT_TYPES == STREAM_EVENT_TYPES
...
async def test_run_populates_tools_used_across_iterations(tmp_path):
    ...
    from atom.agent.hook import AgentHookContext
    from atom.bus.events import OutboundMessage
    from atom.providers.base import ToolCallRequest
...
async def test_run_populates_observability_fields(tmp_path):
    from atom.agent.hook import AgentRunHookContext
    from atom.bus.events import OutboundMessage
...
async def test_run_user_hooks_still_fire_alongside_capture(tmp_path):
    ...
    from atom.agent.hook import AgentHook, AgentHookContext
    from atom.bus.events import OutboundMessage
...
async def test_sdk_capture_prefers_run_level_snapshot():
    from atom.agent.hook import AgentHookContext, AgentRunHookContext, SDKCaptureHook
    from atom.providers.base import ToolCallRequest
...
async def test_session_export_and_restore_preserve_runtime_context(tmp_path):
    ...
    content, marker = append_runtime_context(
        "visible user text",
        [RuntimeContextBlock(source="host_notes", content="stable model-only context")],
    )
