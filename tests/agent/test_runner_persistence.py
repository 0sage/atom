...
async def test_runner_persists_large_tool_results_for_follow_up_calls(tmp_path):
    ...
    assert (tmp_path / ".atom" / "tool-results" / "test_runner" / "call_big.txt").exists()


def test_persist_tool_result_prunes_old_session_buckets(tmp_path):
    from atom.utils.helpers import maybe_persist_tool_result

    root = tmp_path / ".atom" / "tool-results"
...
def test_persist_tool_result_logs_cleanup_failures(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "atom.utils.helpers._cleanup_tool_result_buckets",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("busy")),
    )
    monkeypatch.setattr(
        "atom.utils.helpers.logger.exception",
...
async def test_read_file_result_is_not_offloaded(tmp_path):
    ...
    offload_dir = tmp_path / ".atom" / "tool-results"
...
async def test_runner_keeps_going_when_tool_result_persistence_fails():
    ...
    runner = AgentRunner()
    with patch(
        "atom.agent.context_governance.maybe_persist_tool_result",
