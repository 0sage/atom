...
from atom.providers.base import GenerationSettings, LLMProvider, LLMResponse
from atom.providers.factory import ProviderSnapshot
from atom.sdk.types import SessionSnapshot
...
async def test_streamed_sdk_resolves_session_runtime_after_lock_admission(tmp_path) -> None:
    ...
    await lock.acquire()
    try:
        run = await Atom(loop).run_streamed("hello", session_key=session_key)
