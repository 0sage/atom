...
from atom.providers.base import GenerationSettings, LLMProvider, LLMResponse, ToolCallRequest
...
def _make_loop(tmp_path):
    ...
    with patch("atom.agent.loop.ContextBuilder"), \
         patch("atom.agent.loop.SessionManager"), \
         patch("atom.agent.loop.SubagentManager") as mock_sub_mgr:
        mock_sub_mgr.return_value.cancel_by_session = AsyncMock(return_value=0)
        loop = AgentLoop(bus=bus, provider=provider, workspace=tmp_path)
    return loop


_TURN_SCOPE_ACTIVE: ContextVar[bool] = ContextVar("test_turn_scope_active", default=False)


@contextmanager
def _turn_scope(active: bool):
    """Minimal turn-scoped ContextVar binding, like real turn_scopes entries."""
    token = _TURN_SCOPE_ACTIVE.set(active)
    try:
        yield
    finally:
...
async def test_ephemeral_runner_enters_and_restores_turn_scopes(tmp_path):
    ...
    async def chat_with_retry(**_kwargs):
        assert _TURN_SCOPE_ACTIVE.get() is True
    ...
    await loop._run_agent_loop(
        ...
        ephemeral=True,
        turn_scopes=[_turn_scope(True)],
    )

    assert _TURN_SCOPE_ACTIVE.get() is False
...
@pytest.mark.asyncio
async def test_runtime_context_reaches_model_without_leaking_into_public_history(tmp_path):
    from atom.agent.loop import AgentLoop
    from atom.bus.events import InboundMessage
    from atom.bus.queue import MessageBus
...
async def test_next_turn_after_llm_error_keeps_turn_boundary(tmp_path):
    from atom.agent.loop import AgentLoop
    from atom.agent.runner import _PERSISTED_MODEL_ERROR_PLACEHOLDER
    from atom.bus.events import InboundMessage
    from atom.bus.queue import MessageBus
...
async def test_subagent_max_iterations_announces_existing_fallback(tmp_path, monkeypatch):
    from atom.agent.subagent import SubagentManager, SubagentStatus
    from atom.bus.queue import MessageBus
    ...
    monkeypatch.setattr("atom.agent.tools.filesystem.ListDirTool.execute", fake_execute)
