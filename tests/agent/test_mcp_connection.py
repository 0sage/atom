...
from atom.agent.tools import mcp as mcp_runtime
from atom.agent.tools.base import Tool
from atom.agent.tools.mcp import MCPProvider, MCPResourceWrapper, MCPToolWrapper
from atom.agent.tools.registry import ToolRegistry
from atom.config.loader import load_config, save_config
...
async def test_connect_mcp_retries_when_no_servers_connect(tmp_path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("atom.agent.tools.mcp.connect_mcp_servers", _fake_connect)
...
async def test_connect_mcp_does_not_report_failure_before_oauth_authorization(
    ...
    connect = AsyncMock()
    monkeypatch.setattr("atom.agent.tools.mcp.connect_mcp_servers", connect)
    monkeypatch.setattr(
        "atom.agent.tools.mcp_oauth.mcp_oauth_has_credentials",
