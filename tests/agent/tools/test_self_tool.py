...
from atom.agent.tools.web import WebSearchConfig, WebToolsConfig
...
class TestSubagentHookStatus:
    ...
    async def test_after_iteration_updates_status(self):
        ...
        from atom.agent.subagent import SubagentStatus, _SubagentHook
    ...
    async def test_after_iteration_no_status_is_noop(self):
        ...
        from atom.agent.hook import AgentHookContext
        from atom.agent.subagent import _SubagentHook
...
class TestScratchpadInspection:
    ...
    async def test_inspect_scratchpad_string(self):
        tool = _make_tool()
        await tool.execute(action="set", key="current_project", value="atom")
...
class TestRequestContext:
    ...
    def test_audit_reads_bound_session(self):
        ...
        with patch("atom.agent.tools.self.logger.info") as info:
            with request_context(ctx):
