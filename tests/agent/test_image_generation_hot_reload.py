...
from atom.agent.tools.image_generation import (
    ImageGenerationTool,
...
async def test_image_generation_reload_replaces_and_removes_live_tool(
    ...
    config = load_config()
    config.providers.openai.api_key = "first-key"
    ...
    assert first_tool.provider_configs["openai"].api_key == "first-key"

    config = load_config()
    config.providers.openai.api_key = "second-key"
    ...
    assert second_tool.provider_configs["openai"].api_key == "second-key"
...
async def test_image_generation_reload_reaches_agent_runtime_control(
    ...
    monkeypatch.setattr("atom.config.loader._current_config_path", config_path)
    config = load_config()
    config.providers.openai.api_key = "image-key"
    config.tools.image_generation.enabled = True
    save_config(config)
