from atom.providers.base import ProviderConversationState
...
def test_fork_session_drops_source_runtime_context(tmp_path):
    ...
    assert "host-only guidance" not in model_content
    ...
    session.messages.append(
        {
            "role": "assistant",
            "content": "来了 🎨",
            "media": ["/home/user/.atom/media/generated/img_abc.png"],
        }
    )
    ...
    session.messages.append(
        {
            ...
            "content": (
                ...
                "来了 🎨\n"
                "[image: /home/user/.atom/media/generated/img_old.png]\n\n"
                "generate_image(\"16:9\")\n"
                "message(\"来了 🎨\")"
            ),
        }
...
def test_get_history_respects_max_tokens(monkeypatch):
    ...
    monkeypatch.setattr(
        "atom.session.manager.estimate_message_tokens",
