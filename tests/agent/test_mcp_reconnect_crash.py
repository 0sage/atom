...
lets its idle timeout kill the session, and then exercises atom's MCP
...
from atom.agent.tools import mcp as mcp_module
from atom.agent.tools.mcp import MCPProvider, MCPToolWrapper
from atom.agent.tools.registry import ToolRegistry
from atom.config.schema import MCPServerConfig
from atom.security import network as security_network
...
def allow_loopback_mcp_urls(monkeypatch: pytest.MonkeyPatch):
    """The repro server runs on 127.0.0.1; allow atom to talk to it."""
