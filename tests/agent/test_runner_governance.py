...
from atom.agent.context_governance import (
    BACKFILL_CONTENT,
    ContextGovernanceConfig,
    ContextGovernor,
)
from atom.agent.runner import AgentRunSpec
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr(
        "atom.agent.context_governance.estimate_prompt_tokens_chain",
    ...
    monkeypatch.setattr(
        "atom.agent.context_governance.estimate_message_tokens",
...
def test_snip_history_reserves_budget_for_tool_definitions(monkeypatch):
    ...
    monkeypatch.setattr("atom.agent.context_governance.estimate_prompt_tokens_chain", _estimate)
    token_sizes = {
        "system": 50,
        "old user": 200,
        "old assistant": 200,
...
def test_microcompact_overflow_compacts_to_low_watermark(monkeypatch):
    ...
    monkeypatch.setattr("atom.agent.context_governance.estimate_prompt_tokens_chain", estimate)
...
def test_snip_history_no_user_at_all_falls_back_gracefully(monkeypatch):
    ...
    from atom.providers.base import LLMProvider
