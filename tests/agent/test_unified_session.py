...
from atom.command.builtin import cmd_new, register_builtin_commands
from atom.command.router import CommandContext, CommandRouter
from atom.config.schema import AgentDefaults, Config
from atom.providers.base import GenerationSettings
from atom.session.keys import UNIFIED_SESSION_KEY
from atom.session.manager import Session, SessionManager
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("atom.agent.loop.SessionManager"), \
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates atom onboard)."""
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from atom.agent.memory import Consolidator, MemoryStore
