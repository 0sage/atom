...
class TestConsolidatorSummarize:
    async def test_archive_prompt_includes_media_breadcrumb(
        ...
        path = "/home/user/.atom/media/websocket/upload_photo.png"
    ...
    def test_format_messages_keeps_media_only_user_turn(self):
        path = "/home/user/.atom/media/websocket/clip.mp4"
    ...
    async def test_archive_excludes_model_only_runtime_context(
        ...
        content, marker = append_runtime_context(
            "ship the feature",
            [RuntimeContextBlock(source="host_notes", content="host-only guidance")],
        )
        mock_provider.chat_with_retry.return_value = MagicMock(
        ...
        assert "host-only guidance" not in prompt
...
class TestConsolidatorArchiveErrorHandling:
    ...
    async def test_archive_propagates_template_failure_without_raw_archive(
        ...
        consolidator.store.raw_archive = MagicMock()
        monkeypatch.setattr(
            "atom.agent.memory.render_template",
...
class TestRawArchiveTruncation:
    ...
    def test_raw_archive_excludes_model_only_runtime_context(self, store):
        ...
        assert "host-only guidance" not in entry
