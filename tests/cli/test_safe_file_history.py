...
from atom.cli.commands import SafeFileHistory as LegacySafeFileHistory
from atom.cli.terminal import SafeFileHistory, _sanitize_surrogates
...
class TestSanitizeSurrogates:
    ...
    def test_emoji_already_correct(self):
        ...
        assert _sanitize_surrogates("hello 🐈 atom") == "hello 🐈 atom"
...
class TestSafeFileHistory:
    ...
    def test_emoji_preserved(self, tmp_path):
        hist = SafeFileHistory(str(tmp_path / "history"))
        hist.store_string("hello 🐈 atom")
        entries = list(hist.load_history_strings())
        assert entries[0] == "hello 🐈 atom"
