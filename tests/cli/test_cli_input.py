...
from atom.cli import stream as stream_mod
...
def mock_prompt_session():
    ...
    with patch("atom.cli.terminal._prompt_session", mock_session), \
         patch("atom.cli.terminal.patch_stdout"):
        yield mock_session
...
def test_init_prompt_session_creates_session():
    ...
    with patch("atom.cli.terminal.PromptSession") as mock_session_cls, \
         patch("atom.cli.terminal.FileHistory"), \
...
async def test_print_interactive_progress_line_pauses_spinner_before_printing():
    ...
    with patch("atom.cli.terminal._print_interactive_line", side_effect=fake_print):
        thinking = stream_mod.ThinkingSpinner(console=mock_console)
        with thinking:
