...
class TestRestartCommand:
    ...
    async def test_restart_sends_message_and_calls_execv(self):
        from atom.command.builtin import cmd_restart
        from atom.command.router import CommandContext
        from atom.utils.restart import (
        ...
        with patch.dict(os.environ, {}, clear=False), \
             patch("atom.command.builtin.asyncio", new=fake_asyncio), \
             patch("atom.command.builtin.os.execv") as mock_execv:
    ...
    async def test_restart_exit_mode_does_not_spawn(self):
        ...
        with patch("atom.command.builtin.asyncio", new=fake_asyncio), \
             patch("atom.command.builtin.subprocess.Popen") as mock_popen, \
             patch("atom.command.builtin.os._exit") as mock_exit, \
             patch("atom.command.builtin.os.execv") as mock_execv:
    ...
    async def test_restart_intercepted_in_run_loop(self):
        ...
        with patch.object(loop, "_dispatch", new_callable=AsyncMock) as mock_dispatch, \
             patch("atom.command.builtin.asyncio", new=fake_asyncio), \
             patch("atom.command.builtin.os.execv"):
    ...
    async def test_status_intercepted_in_run_loop(self):
        ...
        with patch.object(loop, "_dispatch", new_callable=AsyncMock) as mock_dispatch:
            ...
            mock_dispatch.assert_not_called()
            assert "atom" in out.content.lower() or "Model" in out.content
    ...
    async def test_run_agent_loop_estimates_usage_when_provider_omits_it(self, monkeypatch):
        loop, _bus = _make_loop()
        monkeypatch.setattr(
            "atom.agent.runner.estimate_prompt_tokens_chain",
            lambda *_args, **_kwargs: (123, "test"),
        )
        monkeypatch.setattr(
            "atom.agent.runner.estimate_message_tokens",
            lambda _message: 7,
        )
