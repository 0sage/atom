# Atom Python SDK: Run an AI Agent from Python

Use atom as a Python library. The SDK gives you the same agent runtime used
...
- atom SDK runs an agent around a model.
...
Install and configure atom first. If you have not done that yet, follow the
...
`Atom.from_config()` reuses your normal `~/.atom/config.json` and
`~/.atom/workspace/`. Provider, model, tools, memory, and session behavior
...
`atom status` should show the config path, workspace path, active model or
...
async def main() -> None:
    async with Atom.from_config() as bot:
...
```python
from atom import STREAM_EVENT_TEXT_DELTA
...
Save this as `sdk_demo.py` after `atom agent -m "Hello!"` works:
...
async def main() -> None:
    prompt = " ".join(sys.argv[1:]) or "Explain what atom is in one paragraph."
    ...
    async with Atom.from_config() as bot:
        ...
        async for event in bot.stream(prompt, session_key=session_key):
            if event.type == STREAM_EVENT_TEXT_DELTA:
                ...
                raise RuntimeError(event.error or "atom run failed")
...
workspace: /Users/alice/.atom/workspace
...
This script shows the usual production shape: create one `Atom`, choose a
...
| Concept | Meaning |
|---------|---------|
| `Atom` | The SDK object that owns one configured agent runtime. |
...
| Memory | Long-term memory files managed by atom. |
...
atom has two programming surfaces:

| Use | Choose | Why |
|-----|--------|-----|
| Python code running in the same process as atom | Python SDK | Direct access to `RunResult`, sessions, memory, runtime helpers, hooks, and stream events. |
...
process isolation, or need to call atom from a non-Python service.
...
async with Atom.from_config(workspace="/my/project") as bot:
    result = await bot.run("Explain the project structure")
```

Use a custom config when you run multiple atom instances or test an isolated
setup:

```python
async with Atom.from_config(
...
The config controls what atom may use. The workspace is where atom keeps
...
```python
bot = Atom.from_config(model="openai/gpt-4.1")
```
...
```python
bot = Atom.from_config(model_preset="fast")
...
provider and model work with `atom agent -m "Hello!"` first.
...
become atom session history. Ingesting a transcript does not call the model,
...
metrics, tracing, or output post-processing without modifying atom internals:

```python
from atom.agent import AgentHook, AgentHookContext
...
### `Atom.from_config(config_path=None, *, workspace=None, model=None, model_preset=None)`

Create a `Atom` instance from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.atom/config.json`. |
...
atom agent loop. A context provider receives a `RequestContext` before each
model turn and may return one or more `RuntimeContextBlock` values. Use
`attributes` for caller-owned routing data; atom keeps it separate from
...
```python
from atom.agent import AgentHook
