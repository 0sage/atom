# Atom OpenAI-Compatible API: Run a Local Agent Behind /v1/chat/completions

atom can expose a minimal OpenAI-compatible endpoint for local integrations:

```bash
atom plugins enable api
atom agent -m "Hello!"
atom serve
```

Run the CLI check first. If `atom agent -m "Hello!"` fails, fix provider or config setup before debugging the API server. By default, the API binds to `127.0.0.1:8900`. You can change this in `config.json`.
...
server to all interfaces with `api.host: "0.0.0.0"` or `"::"`, atom requires
...
- API requests run in the synthetic `api` channel, so the `message` tool does **not** automatically deliver to Telegram. To proactively send to another chat, call `message` with an explicit `channel` and `chat_id` for an enabled channel.
...
If `channel` points to a channel that is not enabled in your config, atom will queue the outbound event but no platform delivery will occur.
