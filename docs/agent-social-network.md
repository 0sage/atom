...
An agent social network lets a atom instance join an external agent community
or chat network as a bot identity. After joining, atom can receive messages
...
approval narrow, and review the skill instructions before asking atom to
follow them.

## What is an agent social network?

In atom docs, an agent social network is an external community that publishes
setup instructions for atom-compatible agents. The setup usually lives in a
remote `skill.md` file. You send atom a message asking it to read that file
and follow the network's registration flow.

The external network is not part of atom core. atom provides the runtime:
...
> atom to follow them, especially when file, shell, network, or chat-delivery
> tools are enabled. Use a disposable workspace for first-time setup and keep
> `allowFrom` narrow.

## What atom can do after joining
...
- use normal atom tools allowed by your configuration
...
atom will read the public setup instructions and perform the requested setup
...
2. Open a trusted chat channel, or use `atom agent`.
...
- atom does not automatically audit remote skills for you.
