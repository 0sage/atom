...
When someone sends a DM to the bot and isn't on the allowlist — whether it's a new user or an existing user on a new channel — atom automatically replies with a **pairing code** (like `ABCD-EFGH`) that expires in 10 minutes. To grant them access:
...
Replace `"Review PR #4502"` with the message you want atom to receive. The
trigger is bound to the session where it was created, so the message goes back
to that same chat. Keep `atom gateway` running so trigger messages can be
...
```bash
printf '%s\n' "Review the latest failed CI job" | atom trigger trg_8K4P2Q9X
```

If an external webhook should wake atom up, run your own small webhook
service and have it call the trigger command after it builds the final message:

```bash
atom trigger <trigger-id> "<message>"
```

If you run multiple atom instances, pass the same config or workspace
...
A session may have multiple triggers, just like it may have multiple scheduled
automations.
...
Periodic background checks are driven by `HEARTBEAT.md` in your workspace (`~/.atom/workspace/HEARTBEAT.md`). When `atom gateway` starts, it registers a protected heartbeat cron job by default. Every 30 minutes, that job checks the file; if it finds tasks under `## Active Tasks`, the agent executes them and delivers only results that pass the notification gate to your most recently active chat channel. If there are no active tasks, or the result is routine with nothing useful to report, the heartbeat is skipped silently.
...
**Setup:** edit `~/.atom/workspace/HEARTBEAT.md` (created automatically by `atom onboard`):
...
You can change the interval or disable the built-in heartbeat in `~/.atom/config.json`:
...
> **Note:** The gateway must be running (`atom gateway`) and you must have chatted with the bot at least once so it knows which channel to deliver to.
