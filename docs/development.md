...
This page collects contributor-facing notes for extending atom. User-facing setup and runtime options live in [`configuration.md`](./configuration.md).

## Adding an LLM Provider

atom uses the provider registry in `atom/providers/registry.py` as the source of truth for LLM provider metadata. Most OpenAI-compatible providers need only two changes.
...
2. Add a field to `ProvidersConfig` in `atom/config/schema.py`:
...
Environment variables, config matching, and provider status derive from those two entries.
...
| `is_gateway` | Provider can route many model families rather than serving one vendor's catalog. |
...
- `atom/audio/transcription_registry.py` owns provider names, aliases, default models, and adapter loading.
- `atom/providers/transcription.py` owns provider-specific HTTP behavior.

Credentials still live under `providers.<provider>` so every surface resolves API keys and API bases the same way.
...
2. Add a `ProviderSpec` in `atom/providers/registry.py`.
...
3. Add an adapter class in `atom/providers/transcription.py`.
...
4. Register the adapter in `atom/audio/transcription_registry.py`.

```python
TranscriptionProviderSpec(
    name="my_stt",
    default_model="my-default-stt-model",
    adapter="atom.providers.transcription:MySTTTranscriptionProvider",
    aliases=("mystt",),
)
```
...
- config schema defaults and aliases in `tests/config/`
