...
This guide shows when to use the Atom Python SDK instead of calling a model
...
- a Python script that creates a `Atom`
...
- Reuse one `Atom` instance for related work.
...
- If SDK code fails, first run `atom agent -m "Hello!"` in the same
  environment.
