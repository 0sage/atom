# How to Configure an OpenAI-Compatible Provider in atom

atom can call OpenAI-compatible model providers by configuring an `apiBase`,
...
Verify the endpoint responds before debugging atom:
...
Merge this into `~/.atom/config.json`:
...
- Do not point atom at untrusted proxy endpoints for private workspaces.

## Troubleshooting

- If `curl /models` fails, fix the provider endpoint before changing atom.
- If atom says the model is unknown, check the model ID expected by the
  provider.
- If auth fails, confirm whether the provider wants Bearer auth and whether the
  key is present in the environment that starts atom.
