# How to Run an OpenAI-Compatible Agent API with atom

atom can expose a local OpenAI-compatible endpoint behind
`/v1/chat/completions`. This lets existing OpenAI-style clients talk to a
tool-using atom agent instead of a raw model.
...
- If `/v1/chat/completions` fails, test `atom agent -m "Hello!"` first.
...
- [Atom OpenAI-Compatible API](../openai-api.md)
