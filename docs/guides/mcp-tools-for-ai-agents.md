# How to Add MCP Tools to an AI Agent with atom

atom can connect MCP servers and expose their tools to the agent alongside
...
publishes an MCP adapter, or when you want a clean boundary between atom and
external tool logic.
...
Add a stdio MCP server to `~/.atom/config.json`:
...
Restart atom, then ask a question that needs the MCP tool.
...
- Keep MCP server install/update steps outside atom config when possible.
...
- Start `atom gateway --verbose` and check MCP startup logs.
- Confirm the MCP command works by itself before debugging atom.
