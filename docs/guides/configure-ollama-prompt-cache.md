# How to Improve Ollama Tool-Calling Prompt Cache Reuse in atom

Some Ollama model templates move or remove tool definitions as a conversation
switches between user, assistant, and tool messages. atom can send a correct
...
`llama3.1:8b` tag with a prefix-stable tool template. It does not modify atom or
...
- a atom model preset that uses the derived tag
...
- atom becomes slow after the model calls a tool;
...
```bash
atom agent --session cli:ollama-cache-check \
...
atom appends an assistant tool call and its result, that user message is no
...
This is a model-template behavior. At the API boundary, atom continues to append
...
## Select the derived model in atom

Merge this preset into `~/.atom/config.json` and select it:
...
```bash
atom status
atom agent --session cli:ollama-stable-check \
  --message "Use the exec tool to calculate 2+2, then answer"
atom agent --session cli:ollama-stable-check \
  --message "Use the exec tool to calculate 4+7, then answer"
```
...
- [Provider Cookbook: Local OpenAI-Compatible Server](../provider-cookbook.md#recipe-local-openai-compatible-server)
- [Providers and Models: Custom OpenAI-Compatible Endpoint](../providers.md#custom-openai-compatible-endpoint)
