# AI Agent Memory in atom

This page explains how atom implements long-term AI agent memory: session
history, compressed archives, durable knowledge files, Dream consolidation, and
Git-backed memory changes.

atom's memory is built on a simple belief: memory should feel alive, but it should not feel chaotic.

Good memory is not a pile of notes. It is a quiet system of attention. It notices what is worth keeping, lets go of what no longer needs the spotlight, and turns lived experience into something calm, durable, and useful.

That is the shape of memory in atom.

## The Design

atom does not treat memory as one giant file.
...
Memory moves through atom in two stages.

### Stage 1: Consolidator

When a conversation grows large enough to pressure the context window, atom does not try to carry every old message forever.
...
This is why atom's memory is not just archival. It is interpretive.

## The Files

In this page, `workspace` means the configured **agent workspace** (the default
is `~/.atom/workspace/`, or the path passed with `--workspace`). Selecting a
different project for a session changes that chat's project context and tool
...
- `SOUL.md` remembers how atom should sound.
...
After Dream changes long-term memory files, atom can record that change with `GitStore`.
...
Dream decides what to keep, update, or forget using atom's built-in memory instructions. Most users can leave this alone.
...
To return to atom's default behavior, delete `prompts/dream.md` or leave it empty.

Each workspace has its own guide. Changing this file does not affect other atom workspaces.
